...
class RightEncoderNode : public rclcpp::Node
{
public:
  RightEncoderNode() : Node("right_encoder_node")
  {
    this->declare_parameter<double>("pulses_per_revolution", 90.0/2.0);
    this->declare_parameter<double>("publish_rate_hz",       20.0);
    this->declare_parameter<std::string>("pulse_topic", "/encoder/right/pulse");
    this->declare_parameter<std::string>("count_topic", "/encoder/right/count");
    this->declare_parameter<std::string>("rpm_topic",   "/encoder/right/rpm");
    ...
    // Subscriber: receives Bool=true on every encoder pulse from Unity
    ...
    // Optional reset: ros2 service call /encoder/right/reset std_srvs/srv/Trigger {}
    reset_srv_ = this->create_service<std_srvs::srv::Trigger>(
      ...
      [this](const std_srvs::srv::Trigger::Request::SharedPtr,
        ...
        res->success = true;
        res->message = "Right encoder count reset to 0.";
        RCLCPP_INFO(this->get_logger(), "Right encoder count reset.");
      });

    RCLCPP_INFO(this->get_logger(),
      "RightEncoderNode ready  |  pulse: %s  |  PPR: %.0f  |  rate: %.1f Hz",
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<RightEncoderNode>());
  rclcpp::shutdown();
  return 0;
}